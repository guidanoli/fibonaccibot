...
def FibonacciBot(event, context):
    ...
    post_msg = FormatMessage( i["counter"], i["value"] )
        
    try:
        post = graph.put_photo(open("bar.png","rb"), message=post_msg)
        print("Post published: \""+post_msg[:40]+"...\".")
        print("ID: "+post["id"])
        if COMMENTS == "true":
          from comments import getComment
          try:
            comment_msg = getComment( i["counter"] , i["value"] )
            comment = graph.put_comment(post["post_id"],comment_msg)
            print("Comment published: \""+comment_msg[:40]+"...\".")
            print("ID: "+comment["id"])
          except facebook.GraphAPIError:
            print("An error occurred while trying to comment post")
            return
    except facebook.GraphAPIError:
